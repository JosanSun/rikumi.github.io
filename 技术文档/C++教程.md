...
| 后置转换尖又圆 | 后置`++` `--` 强制转换 `static/dynamic/reinterpret_cast<type>` `(type)` |
| 前置增删求大小 | 前置`++` `--` `new` `delete` `sizeof`      |
| 反非容址正负针 | 按位反`^` 非`!` 解引用（取内容）`*` 取地址`&` 正负号`+` `-` 指针成员 `.*` `->*` |
| 乘除加减移比较 | 四则运算`*` `/` `+` `-` 移位运算`<<` `>>` 大小比较`<` `<=` `>=` `>` |
| 相等按位与异或 | 相等不等比较`==` `!=` `<>` 按位与`&` 按位异或`^` 按位或`|` |
| 与或三赋抛逗号 | `&&` `||` 三元运算 `?:` 各种赋值 `+=` `-=` `*=` `/=` `%=` `&=` `^=` `|=` `<<=` `>>=` `=` 抛异常`throw` 逗号运算符`,` |
...
如果熟悉运算符重载，可以推断出自增自减运算的实现方式（以自增为例）：
